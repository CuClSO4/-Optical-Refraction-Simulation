...
import { type OpticElement, type Material, defaultPolygonPoints } from "./core/geometry.ts";
import type { ContinuousSpectrum, LightSource, Spectrum } from "./core/trace.ts";
import { type Vec2, v } from "./core/math2d.ts";

export type { Material };

let idSeed = 0;
const nextId = (prefix: string): string => `${prefix}-${(++idSeed).toString(36)}`;

/* -------------------------------------------------------------- 材料预设 */

export const MATERIALS: { key: string; label: string; n: number; abbe: number }[] = [
...
export const HIGH_DISPERSION: Material = { n: 1.72, abbe: 18 };
 ...
 */
export const DISPERSION_GLASS: Material = { n: 1.6, abbe: 22 };

/* ------------------------------------------------------------ 元件工厂 */

export function makeElement(kind: OpticElement["kind"], pos: Vec2, material: Material): OpticElement {
...
export const ELEMENT_KIND_LABELS: { kind: OpticElement["kind"]; label: string }[] = [
...
export function makeSource(overrides: Partial<LightSource> = {}): LightSource {
  return {
    id: nextId("src"),
    label: "光束",
    enabled: true,
...
export interface ScenePreset {
  ...
  label: string;
  hint: string;
  build: () => { elements: OpticElement[]; sources: LightSource[] };
}
...
export const SCENE_PRESETS: ScenePreset[] = [
  ...
  },
  {
    ...
    build: () => ({
      elements: [
        {
          ...(makeElement("plate", v(0, 0), { n: 1.7847, abbe: 25.68 }) as OpticElement),
          halfWidth: 1.4,
          halfHeight: 2.4,
        },
      ],
  ...
  },
  {
    ...
    build: () => ({
      elements: [
        {
          ...(makeElement("polygon", v(1.2, 0), { n: 1.333, abbe: 55.7 }) as OpticElement),
...
/** 生成圆形多边形顶点（近似水珠/圆柱透镜）。 */
export function circlePoints(radius: number, segments: number): Vec2[] {
  const pts: Vec2[] = [];
...
/** 默认场景：凸透镜聚焦。 */
export function defaultScene(): { elements: OpticElement[]; sources: LightSource[] } {
  return SCENE_PRESETS[0]!.build();
}

/** 深拷贝（含嵌套数组），用于历史快照。 */
export function cloneScene(elements: OpticElement[], sources: LightSource[]): {
  elements: OpticElement[];
  sources: LightSource[];
} {
  return {
    elements: JSON.parse(JSON.stringify(elements)) as OpticElement[],
    sources: JSON.parse(JSON.stringify(sources)) as LightSource[],
  };
}
