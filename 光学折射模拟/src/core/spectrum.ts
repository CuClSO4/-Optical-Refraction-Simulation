...
import type { Vec3 } from "./trace.ts";
...
export const LAMBDA_REF: Record<"d" | "F" | "C" | "e" | "g", number> = {
  d: 587.5618, // 氦 d 线
  F: 486.1327, // 氢 F 线（蓝）
  C: 656.2725, // 氢 C 线（红）
  e: 546.074, // 汞 e 线
 ...
 */
export function wavelengthToLinearRGB(lambda: number): Vec3 {
  let r = 0;
  let g = 0;
  let b = 0;
  const l = lambda;
 ...
 */
export function cauchyB(nD: number, abbe: number): number {
  if (!Number.isFinite(abbe) || abbe <= 0) return 0;
  const inv = (l: number): number => 1 / (l * l);
  const delta = inv(LAMBDA_REF.F) - inv(LAMBDA_REF.C);
  return (nD - 1) / (abbe * delta);
}

/** 给定 n_d 与阿贝数，求某波长下的折射率（下限 1）。 */
export function refractiveIndexAt(nD: number, abbe: number, lambda: number): number {
