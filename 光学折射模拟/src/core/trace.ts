...
import { type Vec2, add, dirFrom, dot, norm, perp, scale, sub } from "./math2d.ts";
import { refractiveIndexAt, wavelengthToLinearRGB } from "./spectrum.ts";
...
export type EmissionMode = "collimated" | "area" | "point";
...
export interface LightSource {
  ...
  enabled: boolean;
  pos: Vec2;
  /** 光束轴方向（弧度，世界坐标：+x 向右，+y 向下）。 */
  angle: number;
  mode: EmissionMode;
  /** 光束宽度（世界单位）：决定发射点的横向范围。 */
  beamWidth: number;
  /** 每条光束的光线数/每束光线数。 */
  rayCount: number;
  /** 点光源 / 面光源的发散角（度）。0 表示平行光。 */
  spread: number;
  /** 叠加的光束数量（多束光）。 */
  beamCount: number;
  /** 复色光时相邻光束的波长跨度（nm），用于“多束不同颜色”。 */
  beamTint: number;
  /**
   * 光源标记的显示/命中尺寸倍率（1 = 默认）。
   * 只影响绘制与鼠标命中范围，不参与物理计算；调大可让光源更好点中。
   */
  size: number;
  spectrum: Spectrum;
}

export interface ContinuousSpectrum {
  kind: "continuous";
  /** 采样波长数量：1 为单色。 */
  samples: number;
  lambdaMin: number;
...
export interface DiscreteSpectrum {
  kind: "discrete";
  /** 离散谱线：波长与相对强度。 */
  lines: { lambda: number; weight: number }[];
}

export type Spectrum = ContinuousSpectrum | DiscreteSpectrum;
...
export function fresnel(cosI: number, n1: number, n2: number): FresnelResult {
...
/** 反射方向。 */
export function reflectDirection(d: Vec2, n: Vec2): Vec2 {
...
export function expandSpectrum(s: Spectrum): { lambda: number; weight: number }[] {
  if (s.kind === "discrete") {
  ...
  const out: { lambda: number; weight: number }[] = [];
 ...
 */
export function emitRays(src: LightSource): EmittedRay[] {
  const out: EmittedRay[] = [];
...
export function traceScene(
  ...
  worldBounds: { min: Vec2; max: Vec2 },
  opts: TraceOptions = DEFAULT_TRACE_OPTIONS,
): TraceResult {
  const t0 = performance.now();

  // 预生成几何体，避免每条光线重复构造
  const shapes: { el: OpticElement; geo: ShapeGeometry }[] = [];
...
function countWavelengths(sources: readonly LightSource[]): number {
  let n = 0;
...
function traceRay(
  start: PendingRay,
  shapes: { el: OpticElement; geo: ShapeGeometry }[],
  box: Box,
  opts: TraceOptions,
  stats: TraceStats,
  segments: Segment[],
  ...
  let cur: PendingRay | undefined = start;
...
function findElementHit(
  ...
  shapes: { el: OpticElement; geo: ShapeGeometry }[],
  stats: TraceStats,
): { hit: NonNullable<ReturnType<typeof intersectShape>>; geo: ShapeGeometry } | null {
  let bestT = Infinity;
  let bestHit: ReturnType<typeof intersectShape> = null;
  let bestGeo: ShapeGeometry | null = null;
...
function mediumIndices(
  hit: NonNullable<ReturnType<typeof intersectShape>>,
  geo: ShapeGeometry,
  d: Vec2,
  lambda: number,
): { n1: number; n2: number; eta: number; exiting: boolean } {
...
function rayBoxExit(o: Vec2, d: Vec2, box: Box): number {
  let tExit = Infinity;
  const oa = [o.x, o.y];
  const da = [d.x, d.y];
...
export function elementIndexAt(el: OpticElement, lambda: number): number {
