...
export interface Material {
  /** 参考波长（587.6nm，氦 d 线）下的折射率 n_d。 */
  n: number;
  /** 阿贝数 V_d：越大色散越弱（设成 10000 即近似无色散）。 */
  abbe: number;
}
...
export interface ArcLensElement extends BaseElement {
  kind: "biconvex" | "planoconvex" | "biconcave" | "meniscus";
  /** 通光半径（半高，世界单位）。 */
  height: number;
  /** 中心厚度（世界单位）。 */
  centerThickness: number;
  /** 前表面弦高，>0 表示弧顶朝 +x（向光侧凸出）。 */
  sagFront: number;
  /** 后表面弦高，>0 表示弧顶朝 −x。 */
  sagBack: number;
}

export interface PlateElement extends BaseElement {
  kind: "plate";
  halfWidth: number;
  halfHeight: number;
}
...
function arcSpec(h: number, sag: number): ArcSpec {
 ...
 */
function arcPoints(h: number, sag: number, sigma: 1 | -1, steps: number): Vec2[] {
...
function sagOf(h: number, sag: number): number {
  const spec = arcSpec(h, sag);
 ...
 */
function resolveLens(el: ArcLensElement): {
  h: number;
  tc: number;
  sF: number;
  sB: number;
...
function biconvexOutline(el: ArcLensElement): Vec2[] {
...
function planoconvexOutline(el: ArcLensElement): Vec2[] {
...
function biconcaveOutline(el: ArcLensElement): Vec2[] {
  const { h, tc, sF, sB } = resolveLens(el);
 ...
 */
function meniscusOutline(el: ArcLensElement): Vec2[] {
  const { h, tc, sF, sB } = resolveLens(el);
...
function prismOutline(apexDeg: number, size: number): Vec2[] {
...
/** 矩形轮廓。 */
function rectOutline(halfWidth: number, halfHeight: number): Vec2[] {
...
/** 默认自定义多边形（正五边形，CCW）。 */
export function defaultPolygonPoints(halfSize = 1.3): Vec2[] {
  const pts: Vec2[] = [];
  const n = 5;
  for (let i = 0; i < n; i++) {
...
/** 生成任意光学元件的世界坐标几何。 */
export function buildGeometry(el: OpticElement): ShapeGeometry {
  let local: Vec2[];
...
export function insideElement(p: Vec2, geo: ShapeGeometry): boolean {
  return pointInPolygon(p, geo.outline);
}

/** 沿光线反方向退极小距离取探针点，避免恰好落在边界上。 */
export function previousSideProbe(hit: Hit, dir: Vec2): Vec2 {
