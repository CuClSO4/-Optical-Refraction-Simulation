...
import type { OpticElement } from "./core/geometry.ts";
import { LAMBDA_REF, wavelengthToLinearRGB } from "./core/spectrum.ts";
import type { ContinuousSpectrum, LightSource, Spectrum } from "./core/trace.ts";
import { expandSpectrum } from "./core/trace.ts";
import {
  ELEMENT_KIND_LABELS,
  MATERIALS,
  SPECTRUM_PRESETS,
...
import { QUALITY_MAX as Q_MAX, QUALITY_MIN as Q_MIN } from "./render.ts";
...
export interface PanelCallbacks {
  ...
  rebuild: () => void;
  refresh: () => void;
  update: (mutate: (s: AppState) => void, options?: { history?: boolean; rebuild?: boolean }) => void;
  invalidate?: () => void;
  ...
  addElement: (kind: OpticElement["kind"]) => void;
  addSource: () => void;
...
export class Panel {
  ...
  private current: Controls | null = null;
  /** 右侧「属性菜单」内容的构建函数（重建时复用）。 */
  private selectionBody: (() => void) | null = null;
   ...
   */
  private buildSelectionSection(host: HTMLElement): Controls {
    const doc = host.ownerDocument ?? document;
    ...
    const buildBody = (): void => {
      ...
      const mk = (label: string, title2: string, danger: boolean, fn: () => void): HTMLButtonElement => {
  ...
  private buildElementsSection(host: HTMLElement): Controls {
    const doc = host.ownerDocument ?? document;
    const refreshers: (() => void)[] = [];
  ...
  private selected(): OpticElement | undefined {
    return this.cb.state.elements.find((e) => e.id === this.cb.state.selectedElementId);
  }

  private elementParamNodes(el: OpticElement, refreshers: (() => void)[]): HTMLElement[] {
    const doc = this.root.ownerDocument;
    const nodes: HTMLElement[] = [];
    const upd = (fn: (target: OpticElement) => void, options?: { rebuild?: boolean }): void => {
      this.cb.update(
        (s) => {
    ...
    const noDispersion = (): boolean => this.live(el).material.abbe >= 900;
    nodes.push(
      numberRow(doc, [
        {
          label: "折射率 n_d",
  ...
  private live<T extends OpticElement>(fallback: T): T {
    return (this.cb.state.elements.find((e) => e.id === fallback.id) as T | undefined) ?? fallback;
  }

  private shapeParamNodes(el: OpticElement, refreshers: (() => void)[]): NumberField[] {
    switch (el.kind) {
      case "biconvex":
      case "biconcave":
      case "meniscus":
  ...
  private field(
    ...
    el: OpticElement,
    refreshers: (() => void)[],
    get: (t: OpticElement) => number,
    sliderMin: number,
    sliderMax: number,
    step: number,
    set: (t: OpticElement, val: number) => void,
  ): NumberField {
    return {
      label,
  ...
  private buildSourcesSection(host: HTMLElement): Controls {
    const doc = host.ownerDocument ?? document;
    const list = doc.createElement("div");
    list.className = "el-list";
    const listRefresh = (): void => {
  ...
  private sourceParamNodes(src: LightSource, refreshers: (() => void)[]): HTMLElement[] {
    const doc = this.root.ownerDocument;
    const nodes: HTMLElement[] = [];
    const live = (): LightSource => this.cb.state.sources.find((s) => s.id === src.id) ?? src;
    const upd = (fn: (t: LightSource) => void, options?: { rebuild?: boolean }): void => {
      this.cb.update((s) => {
        const t = s.sources.find((x) => x.id === src.id);
        if (t) fn(t);
      }, options);
    ...
    const modes: { value: LightSource["mode"]; label: string }[] = [
    ...
    modeSelect.addEventListener("change", () =>
      upd((t) => (t.mode = modeSelect.value as LightSource["mode"])),
    );
    ...
    if (isMono) {
        ...
        numberRow(doc, [
          {
            label: "波长 nm",
            value: () => {
              const s = live().spectrum as ContinuousSpectrum;
              return s.lambda;
            },
            sliderMin: 380,
            sliderMax: 780,
            ...
            onInput: (val) =>
              upd((t) => {
                const s = t.spectrum as ContinuousSpectrum;
      ...
      nodes.push(
        numberRow(doc, [
          {
            label: "采样数",
            value: () => (live().spectrum as ContinuousSpectrum).samples,
            sliderMin: 2,
            sliderMax: 41,
            step: 1,
          ...
          {
            label: "起始 nm",
            value: () => (live().spectrum as ContinuousSpectrum).lambdaMin,
            sliderMin: 380,
            sliderMax: 700,
            step: 5,
            format: (x) => x.toFixed(0),
          ...
          {
            label: "终止 nm",
            value: () => (live().spectrum as ContinuousSpectrum).lambdaMax,
            sliderMin: 420,
            sliderMax: 780,
            step: 5,
            format: (x) => x.toFixed(0),
  ...
  private buildRenderSection(host: HTMLElement): Controls {
    const doc = host.ownerDocument ?? document;
    const refreshers: (() => void)[] = [];
    const nodes: HTMLElement[] = [];

    const r = (): AppState["render"] => this.cb.state.render;
    const t = (): AppState["trace"] => this.cb.state.trace;
...
interface NumberField {
  ...
  /**
   * 滑杆范围（**只管滑杆**）。
   * 数值框的手动输入不受它限制：滑杆只是粗调，详细数值请直接输入。
   */
  sliderMin: number;
  sliderMax: number;
  step: number;
  /** 可选：手动输入的硬边界。省略 = 想填多少填多少（只受数据类型限制）。 */
  hardMin?: number;
  hardMax?: number;
 ...
 */
function isArc(el: OpticElement): boolean {
  return (
    el.kind === "biconvex" ||
    el.kind === "planoconvex" ||
    el.kind === "biconcave" ||
...
function section(title: string, children: (Node | string)[]): HTMLDetailsElement {
...
function row(doc: Document, ...children: (Node | string)[]): HTMLElement {
...
function numberRow(doc: Document, fields: NumberField[]): HTMLElement {
  ...
  for (const field of fields) {
    ...
    const fmt = field.format ?? ((x: number) => (Number.isInteger(x) ? x.toFixed(0) : x.toFixed(2)));
...
function toggleGrid(
  doc: Document,
  items: { label: string; value: () => boolean; onToggle: (val: boolean) => void; refresh: (() => void)[] }[],
): HTMLElement {
  const wrap = doc.createElement("div");
...
function spectrumSwatch(doc: Document, spec: Spectrum, presetKey: string): HTMLElement {
  const wrap = doc.createElement("div");
