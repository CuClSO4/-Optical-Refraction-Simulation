...
import { DEFAULT_RENDER_SETTINGS, type RenderSettings, Renderer } from "./render.ts";
import {
  type ScenePreset,
  SCENE_PRESETS,
  cloneScene,
...
import { Library } from "./library-ui.ts";
import { templateIsSource, templateMaterial, type LibraryTemplate } from "./library.ts";
import { Viewport } from "./view.ts";
...
interface DragState {
  kind: "source" | "source-angle" | "element" | "vertex" | "sag" | "view";
  id: string;
  /** 抓取点与元件中心的世界坐标偏移。 */
  grabOffset: Vec2;
  startWorld: Vec2;
...
export class App {
  ...
  private readonly past: { elements: OpticElement[]; sources: LightSource[] }[] = [];
  private readonly future: { elements: OpticElement[]; sources: LightSource[] }[] = [];

  private segments: Segment[] = [];
  /** 当前追踪所用世界范围（与相机解耦，见 computeTraceBounds）。 */
  private traceBounds: { min: Vec2; max: Vec2 } | null = null;
  ...
  /** 从光源模板创建光源。 */
  private addSourceFromTemplate(tpl: LibraryTemplate): void {
  ...
  /** 只做一次尺寸测量（构造期与重试期共用）。 */
  private observeSizeMeasure(): void {
    const rect = this.canvas.getBoundingClientRect();
    const w = rect.width;
    const h = rect.height;
  ...
  update(
    mutate: (s: AppState) => void,
    options: { history?: boolean; rebuild?: boolean; retrace?: boolean } = {},
  ): void {
   ...
   */
  private computeTraceBounds(): { min: Vec2; max: Vec2 } {
  ...
  private refreshTraceBounds(): void {
  ...
  addSource(): void {
    ...
    const src = makeSource({
      ...
      angle: el?.angle ?? 0,
      spectrum: el ? (JSON.parse(JSON.stringify(el.spectrum)) as LightSource["spectrum"]) : undefined,
    });
    this.update((s) => {
      s.sources.push(src);
  ...
  duplicateSelected(): void {
    const el = this.selectedElement();
    if (el) {
      const copy: OpticElement = JSON.parse(JSON.stringify(el)) as OpticElement;
    ...
    const src = this.selectedSource();
    if (!src) return;
    const copy: LightSource = JSON.parse(JSON.stringify(src)) as LightSource;
    copy.id = `dup-${Math.random().toString(36).slice(2, 8)}`;
  ...
  selectedElement(): OpticElement | undefined {
    return this.state.elements.find((e) => e.id === this.state.selectedElementId);
  }

  selectedSource(): LightSource | undefined {
  ...
  private pointerScreen(ev: PointerEvent): Vec2 {
    const rect = this.canvas.getBoundingClientRect();
    return v(ev.clientX - rect.left, ev.clientY - rect.top);
  }

  private pointerWorld(ev: PointerEvent): Vec2 {
    return this.view.toWorld(this.pointerScreen(ev));
  }

  /** 平移视图（拖动空白处 / 中键拖动 / 空格+拖动）。 */
  private startViewDrag(ev: PointerEvent, id = "view"): void {
    const rect = this.canvas.getBoundingClientRect();
    this.dragging = {
      kind: "view",
      id,
  ...
  panViewBy(dxScreen: number, dyScreen: number): void {
  ...
  /** 缩放某个元件的尺寸（Shift + 滚轮）。 */
  private scaleElement(el: OpticElement, factor: number): void {
    this.update(
      (s) => {
        const target = s.elements.find((e) => e.id === el.id);
        if (!target) return;
  ...
  private setInteraction(active: boolean): void {
  ...
  private sourceHitRadius(src: LightSource): number {
    const px = this.view.pixelsPerUnit;
    const sc = src.size && src.size > 0 ? src.size : 1;
  ...
  private hitElement(world: Vec2, geoCache?: (el: OpticElement) => ShapeGeometry): OpticElement | undefined {
    let best: OpticElement | undefined;
  ...
  segmentsSnapshot(): { ax: number; ay: number; bx: number; by: number; power: number }[] {
...
let crashHost: HTMLElement | null = null;
let lastReport = "";
let errorCount = 0;

function crashElement(): HTMLElement | null {
  if (!crashHost && typeof document !== "undefined") crashHost = document.getElementById("crash");
  return crashHost;
}

/** 渲染成功一帧后隐藏错误/启动横幅。 */
export function clearCrashBanner(): void {
...
export function errorTotal(): number {
  return errorCount;
}
...
function installGlobalErrorHandlers(): void {
  ...
  window.addEventListener("unhandledrejection", (ev) => {
    reportError((ev as PromiseRejectionEvent).reason, "未处理的 Promise 拒绝");
  });
}

/* ------------------------------------------------------------------ 工具 */

function isArcLens(el: OpticElement): boolean {
  return (
    el.kind === "biconvex" ||
    el.kind === "planoconvex" ||
    el.kind === "biconcave" ||
...
function rotateInv(p: Vec2, rot: number): Vec2 {
  const c = Math.cos(-rot);
  const s = Math.sin(-rot);
  return v(p.x * c - p.y * s, p.x * s + p.y * c);
}

function pointInShape(p: Vec2, poly: readonly Vec2[]): boolean {
  let inside = false;
...
export function boot(): App {
  ...
  (window as unknown as Record<string, unknown>)["opticsLab"] = app;
  console.info("[光学折射实验室] 启动完成");
  return app;
}

/** 启动并做「是否真的渲染出第一帧」的自检。 */
export function bootWithSelfCheck(): App | null {
  let app: App;
  try {
    app = boot();
  } catch (err) {
