...
import type { ElementKind, OpticElement } from "./core/geometry.ts";
import type { Material } from "./core/geometry.ts";
import { MATERIALS } from "./scene.ts";
...
export interface LibraryTemplate {
  ...
  /** 一句话说明，显示为卡片 tooltip。 */
  hint: string;
  /** 光学元件的形状；光源模板用 "source"。 */
  kind: ElementKind | "source";
  /** 30×24 视图框内的二维示意图形（SVG 内容）。 */
  icon: string;
  /** 点击后应用到新元器件上的覆盖参数（元件参数）。 */
  overrides?: Record<string, unknown>;
  /** 覆盖材质（不写则用当前选中元件的材质，或默认 BK7）。 */
  materialKey?: string;
  /** 光源模板的初始参数。 */
  source?: {
    mode?: "collimated" | "area" | "point";
    lambda?: number;
    samples?: number;
    spread?: number;
    beamCount?: number;
...
function lensIcon(dir: 1 | -1): string {
  const r = 12.5;
  const s = 3.6;
  const bulge = dir * s;
  return `
...
/** 取某个模板的材质。 */
export function templateMaterial(tpl: LibraryTemplate): Material | null {
...
export function templateIsSource(tpl: LibraryTemplate): boolean {
